...
export async function GET(req: NextRequest) {
  try {
    ...
    // Inisialisasi parameter untuk pre-warming cache tingkat nasional tahun berjalan
    const cacheKey = {
      province: 'NASIONAL',
    ...
    console.log('[CRON] Warming AI Insight cache for:', cacheKey)
...
`
      ...
      if (response.ok) {
        ...
        if (textResponse) {
          ...
          return NextResponse.json({ success: true, message: 'Cache updated successfully via Gemini.' })
        }
      }
    }

    // Heuristics Fallback jika API Key kosong/gagal
    const summary = `Analisis tata kelola Posyandu di wilayah NASIONAL (${defaultYear}) menunjukkan keaktifan operasional bulanan sebesar ${pctAktif}%.`
    ...
    const detailedAnalysis = `# Analisis Penilaian Kepatuhan & Keaktifan Layanan Posyandu - NASIONAL (Cron)
...
`
    ...
    return NextResponse.json({ success: true, message: 'Cache updated successfully via fallback heuristics.' })
  } catch (error) {
